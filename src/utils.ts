...
export class PinyinUtils {
    ...
    for (const char of pinyin) {
      if (toneMap[char]) {
        ...
        if (toneMatch) {
          ...
          if ('āēīōūǖ'.includes(toneChar)) toneNumber = 1;
          else if ('áéíóúǘ'.includes(toneChar)) toneNumber = 2;
          else if ('ǎěǐǒǔǚ'.includes(toneChar)) toneNumber = 3;
          else if ('àèìòùǜ'.includes(toneChar)) toneNumber = 4;
        }
      } else {
        result += char;
      }
  ...
  static numberToToneMark(pinyin: string): string {
    ...
    if (tone < 0 || tone > 4) return base;

    // 根据拼音规则，确定哪个元音应该带声调
    const targetVowel = this.findToneVowel(base);
    if (!targetVowel) return base;
   ...
   */
  private static findToneVowel(pinyin: string): string | null {
